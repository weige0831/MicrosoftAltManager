...
export default function HomePage() {
  ...
  return (
    <div className="w-full bg-background text-foreground">
      {/* Simple header (no new-api hooks that crash) */}
      <header className="pointer-events-none fixed inset-x-0 top-0 z-50">
        <div className="pointer-events-auto mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
          ...
          <div className="flex items-center gap-0.5">
            {navLinks.map((l) => (
              <a key={l.href} href={l.href} className="hidden rounded-lg px-3 py-1.5 text-[13px] font-medium text-muted-foreground transition-colors hover:text-foreground sm:block">
                {l.title}
              </a>
            ))}
            <LanguageSwitcher />
            <button onClick={() => setTheme(isDark ? "light" : "dark")} className="grid size-9 place-items-center rounded-lg text-muted-foreground hover:bg-accent">
              {isDark ? <Sun className="size-4" /> : <Moon className="size-4" />}
            </button>
            <Button size="sm" className="ml-2 h-8 rounded-lg px-3.5 text-xs font-medium" onClick={() => nav(primaryTo)}>
              {primaryLabel}
            </Button>
          </div>
        </div>
      ...
      <section className="relative z-10 overflow-hidden px-6 pb-16 pt-24 md:pb-24 md:pt-32 lg:pb-28 lg:pt-36">
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="landing-animate-fade-up mt-8 flex flex-wrap items-center gap-3" style={{ animationDelay: '180ms' }}>
            ...
            <Button variant="outline" className="h-11 rounded-lg px-5 text-sm font-medium" onClick={() => nav("/login")}>
              {t("home.ctaSecondary")}
            </Button>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-px overflow-hidden rounded-xl border border-border/40 bg-border/40 md:grid-cols-3">
            {[
              ...
              <FadeIn key={f.n} className={`group bg-background p-7 transition-colors hover:bg-muted/20 md:p-8 ${f.s}`}>
          ...
          <div className="mt-12 grid grid-cols-2 gap-8 md:grid-cols-4 md:gap-12">
            {[
              ...
              <FadeIn key={i} delay={i * 100} className="group flex flex-col items-center text-center">
      ...
      <section className="px-6 py-24 md:py-32">
        <FadeIn className="mx-auto max-w-2xl text-center" animation="scale-in">
          ...
          <div className="mt-8 flex items-center justify-center gap-3">
            <Button className="group rounded-lg" onClick={() => nav(primaryTo)}>{primaryLabel}<ArrowRight className="ml-1 size-3.5 group-hover:translate-x-0.5" /></Button>
